...
namespace ft {
	...
	template <
		...
		typename NAlloc = std::allocator<TreeNode<T> > >
	class RBTree
	{
	public:
	...
	public:
		...
		pair<iterator, bool> insert(const_reference val) 
		{
			...
			TreeNode<value_type> *no = _nodeAllocator.allocate(1);
			_nodeAllocator.construct(no, TreeNode<value_type>(p));
			...
			TreeNode<value_type> *res = _move(val);
		...
		void erase(iterator it) 
		{
			_size--;
			_root->parent = NULL;
			TreeNode<value_type> *no = _erase(it.base());
			_balance(no);
			if(_root)
			{
				_root->parent = _end;
		...
		{
			TreeNode<value_type> *tmp = _move(value);
		...
		{ 
			return iterator(_upper_bound(value, _root));
		}

		const_iterator	upper_bound(const value_type &value) const
		{
			return const_iterator(_upper_bound(value, _root));
		}

		iterator		lower_bound(const value_type &value)
		{
			return iterator(_lower_bound(value, _root));
		}

		const_iterator	lower_bound(const value_type &value) const
		{
			return const_iterator(_lower_bound(value, _root));
		}
	...
	private:
		void _clear(TreeNode<value_type> *node) 
		{
			if (!node)
				return;
			_clear(node->right);
		...
		TreeNode<value_type> *_move(const_reference val) const 
		{
			TreeNode<value_type> *x = _root;
			TreeNode<value_type> *y = NULL;
		...
		TreeNode<value_type> *_lower_bound(value_type const &value, TreeNode<value_type> *node) const 
		{
			if (!node)
				return(_end);
			...
			{
				TreeNode<value_type> *left = _lower_bound(value, node->left);
				return(left != _end ? left : node);
			}
		}

		TreeNode<value_type> *_upper_bound(value_type const &value, TreeNode<value_type> *node) const 
		{
			if (!node)
				return(_end);
			...
			{
				TreeNode<value_type> *left = _upper_bound(value, node->left);
				return(left != _end ? left : node);
			}
		}

		void _balance(TreeNode<value_type> *node) 
		{
			if(!node)
				return;
			TreeNode<value_type> *parent = NULL;
			TreeNode<value_type> *grandpa = NULL;
			TreeNode<value_type> *uncle = NULL;

			while ((node != _root) && (node->color != false) &&	(node->parent->color == true))
			{
				parent = node->parent;
		...
		void _rotate_left(TreeNode<value_type> *node) 
		{
			TreeNode<value_type> *right = node->right;
		...
		void _rotate_right(TreeNode<value_type> *node) 
		{
			TreeNode<value_type> *left = node->left;
		...
		TreeNode<value_type> *_erase(TreeNode<value_type> *node) 
		{
			if (!node)
				return(NULL);

			TreeNode<value_type> *tmp = NULL;
		...
		void _clear_for_erase(TreeNode<value_type> *node) 
		{
			if (!node)
				return;
	...
	private:
		...
		TreeNode<value_type>		*_end;
		TreeNode<value_type>		*_root;
		size_type					_size;
	};
}
